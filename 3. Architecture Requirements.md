...
### 1.0.1 Public navigation contract

The Render root URL is the canonical public landing-page entry point:

`https://pulse360-gkt8.onrender.com/`

The root Next.js page renders the landing experience. `landing.html` is an internal asset and must not be required in customer-facing links. Authentication begins at `/login`, with navigation back to the root homepage.
...
The landing-page integration and root-route deployment were completed on `main`. The final verified homepage update is commit `ca84ce0`, following the homepage integration and PR #61 merge. This completed the first prerequisite before beginning the FAQ assistant's Supabase `pgvector` implementation.
