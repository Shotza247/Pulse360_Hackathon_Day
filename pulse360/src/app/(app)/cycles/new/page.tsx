  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/cycles", {
        ...
        body: JSON.stringify({ name, description, startDate, endDate, minNominees, maxNominees }),
      });
    ...
    <div className="max-w-xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Cycle Description</label>
            <textarea value={description} maxLength={500} onChange={e => setDescription(e.target.value)}
              placeholder="Briefly describe the focus, scope, or purpose of this review cycle"
              rows={4}
              className="w-full resize-y rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-[#0f1f3d] focus:outline-none focus:ring-2 focus:ring-[#0f1f3d]/20" />
